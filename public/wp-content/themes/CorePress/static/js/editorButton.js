(function () {
    ...
    tinymce.PluginManager.add('gh-addShortCode', function (editor, url) {
        editor.on("click", function (e) {
            var dom = $(e.toElement).parents();
            if ($(dom).attr('id') == 'corepress-editbtn') {
                var code = dom.find("#corepress-editbtn").parent().find('code').text();
                var codedom = dom.find("#corepress-editbtn").parent().find('code');
            ...
            if (e.toElement.tagName == 'PRE') {
                $(e.toElement).append('<div id="corepress-editbtn" title="编辑"><span class="dashicons dashicons-welcome-write-blog"></span></div>');
                $(e.toElement).append('<div id="corepress-delbtn" title="删除"><span class="dashicons dashicons-no"></span></div>');
            }
        })
    });
})();
