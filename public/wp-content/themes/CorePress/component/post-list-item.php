...
if ($postitem['post_meta']['closesummary'] == 1) {
    $postitem['content'] == '';
} else {
    if (has_excerpt()) {
...
if (has_shortcode($postitem['content'], 'pwdshow')) {
    $postitem['content'] = '内容已加密';
}
if (post_password_required()) {
    $postitem['content'] = '内容已加密，请输入密码以后查看';
}
$postitem['thumbnail'] = null;
if ($set['post']['closethumbnail'] != 1) {
...
$postitem['author_avatar'] = '<img alt="" src="' . corepress_get_avatar_url(get_the_author_meta('email'), 24) . '"  class="post-item-avatar">';

$postitem['commentsnum'] = get_comments_number();
$postitem['title'] = get_the_title();
$postitem['category'] = get_the_category();
$postitem['time'] = get_the_time('Y-m-d h:i:s');
$newpost = '';
...
if ($set['post']['closethumbnail'] == 1) {
                ...
                <div class="post-item-info">
                    <div class="post-item-meta">
                        <div class="post-item-meta-item">
                            ...
                            <span class="post-item-time"><i class="fas fa-calendar-alt"></i><?php echo diffBetweenTwoDay($postitem['time']); ?></span>
                        </div>
    ...
    <?php
} else if ($postitem['post_meta']['postshow'] == 1) {
    ?>
